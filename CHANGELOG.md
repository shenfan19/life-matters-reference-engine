...
- `simulation.schedules` 与 `simulation.plans` 一度共存，`plans` 优先，`papers/` 目录禁止混用两者，对应 ADR 0087；随后进一步强制规范到位置层面，仿真固定用 `plans`，优化固定用 `optimization.startpoint`，对应 ADR 0109，取代 ADR 0087。
...
- `optimization.schedules` 统一为决策变量与固定背景量合并的单一列表格式，对应 ADR 0088，后由 ADR 0109 的位置规范取代。
...
- 内部实现改名 `regimen_runner.py` 为 `schedule_runner.py`，API 契约字段名不变，对应 ADR 0116；API/YAML 概念层命名最终维持 `regimen`，`optimization.results.reference` 改名为 `recommended` 并删除中间解码字典，对应 ADR 0117。
...
- `optimization.results` 改为内嵌设计，配合后端无状态服务架构，对应 ADR 0069。
...
- Sim 执行核心在 CLI 与 GUI 之间合并为共用的 `advance_steps`，CLI 同时新增 `--mc-runs`/`--seed` 能力，对应 ADR 0113；随后 CLI 侧的 MC 命令行参数被移除，MC 配置改为只读 YAML 里的 `simulation.mc`，与 `optimization.mc` 模式统一，对应 ADR 0114，修订 ADR 0113。
