...
def test_evidence_name_colliding_with_variable_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_evidence_name_collision'])
...
def test_evidence_applies_to_missing_baseline_ref_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_evidence_missing_baseline_ref'])
