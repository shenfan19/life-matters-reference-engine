...
Each fixture in models/test/invalid/ is intentionally broken in exactly one
...
None/False (see models/test/invalid/README.md and the LoaderEngine.fetch()
...
def test_invalid_step_size_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_step_size'])
...
def test_optimizer_missing_method_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_optimizer_missing_method'])
    assert 'method' in engine.loader.last_error


def test_formula_undefined_variable_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_formula_undefined_var'])
    assert 'undeclared_var' in engine.loader.last_error


def test_formula_deprecated_dt_symbol_is_rejected():
    engine = _make_engine()
    assert not engine.load_models(['test/invalid/test_invalid_formula_deprecated_dt'])
