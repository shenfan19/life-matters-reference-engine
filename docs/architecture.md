...
以下是从前期研究中提取的优化指标建议（基于打靶法或统计法）：

1. **寿命最长**：以模型中的寿命变量（如预期寿命或生存年限）为核心指标，最大化健康年限。适用场景：慢性病（如肺癌或糖尿病）健康模拟。
2. **赚钱最多**：聚焦经济状态（如累计财富或收入增长率），优化职业或教育投资。适用场景：微观社会学与经济学仿真。
3. **生活品质最高（健康最高）**：综合健康指标（如焦虑水平、主观幸福感）。适用场景：交互式人生模拟（Experience Game）。
4. **事业最成功**：职业成就参数（晋升率、影响力指数）。适用场景：社会网络演化对职业发展的作用。
5. **环境可持续性最高**：碳足迹或生态健康指数。适用场景：环境相关交叉行为影响。

## 附录2：用户工作区与存档结构 (users/)

在多用户/玩家环境下，`LifeMatters` 提供独立的用户数据目录，允许分离存档与优化日志：

```
users/
├── user_sample/
│   ├── user_settings.json       # 该用户通用设置
│   ├── simulation/              # 游戏体验存档 (包含 state.json, log 等)
│   └── optimization/            # 参数优化任务数据
└── user_simulator/
```

这确保了在Web或桌面端中不同用户的跑批任务和体验结果互相隔离，随时可用模板创建新的研究区 (`cp -r users/template users/my_study`)。
