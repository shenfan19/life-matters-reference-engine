...
- **基础字号**：14px（gui 已设定为默认）。正文内容不小于 12px，标注/辅助信息不小于 11px，禁止使用 10px 以下字号显示正式内容。
...
- **新增组件**：先在 `gui/public/locales/zh-CN.json` 和 `en-US.json` 中添加对应 key，再在组件中引用。
...
- **步长（step）**：仍使用数值 + 单位（分钟/小时/天），因为步长是计算精度参数，与日历无关。
...
- **可持久化原则**：用户主动做出的选择（下拉框选项、坐标轴/分组切换、过滤条件、视图配置等）默认应视为"需要跨刷新页面、跨切换模型再切回来保留"，不能只用组件内 `useState` 存放——组件重新挂载或页面刷新会让这类状态悄悄退回默认值，用户会感知为"设置丢了"。gui 已有落地机制：并入现有的 per-model session 持久化（`types.ts` 的 `ModelSession` 类型 + `localStorage` 的 `lm_model_sessions`，串联 `useSession.ts`／`usePersistedUI.ts`／`useModelInit.ts` 三处：定义字段、持久化写入、模型加载时的会话优先/YAML 兜底恢复），新增可选择/可配置状态时直接扩展这一条链路，不要另起一份不持久化的 local state。game 若无同名机制，按同一原则建立等价的持久化通道。
  - 例外：真正瞬时的展示态不必持久化，例如 hover tooltip、拖拽中间态、loading spinner。
