...
|--------|---------|---------------------|
| **Sim** | 当前运行时间序列 | 新增曲线叠加显示（用于对比不同步长、不同参数） |
...
YAML 下载**永远不覆盖源文件**（另存为语义）。Opt 标签页下载时提供两个选项：

- **含 Pareto 结果**：将 `optimizer.results` 块写入 YAML 副本（用于发布、归档、与他人共享）
- **不含结果**：下载纯模型定义（用于清洁复制或重新分发模型）
