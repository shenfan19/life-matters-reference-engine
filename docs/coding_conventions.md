...
> 适用范围：`sim_cli/`、`sim_engine/`、`sim_gui/` 的 Python 与 TypeScript/JSX 源码。
...
4. **闭包是否在偷偷扮演独立函数？** 函数体内定义的多个闭包（如 `run_optimizer` 内部的 `_build_schedule_events`/`_clone`/`evaluate`），如果各自有清晰独立的输入输出、且不依赖外层局部变量做隐式状态共享，应提升为顶层函数或独立模块，而不是无限期留在闭包里。
