...
LifeMatters 采用混合架构，包含离线工具（Generator、Optimizer、Loader）和运行时服务（Simulator），通过共享模型库实现高效协作。**架构分工明确：GUI 负责实时控制和复杂交互，CLI 专注于自动化批处理和文件驱动的脚本化任务。**框架满足以下需求：
...
- **混合架构与职责分离**：离线 CLI 工具（Loader/Generator/Optimizer）高效简洁，专注于**自动化批处理和脚本化**；运行时服务（Simulator）配合 GUI 支持**动态控制和复杂实时交互**。
...
- **职责分离**：**GUI/Web 界面**专用于**复杂、实时的交互和控制**；CLI 工具专用于**非交互式的自动化、脚本化任务**（如模型生成、优化、长时间仿真）。
