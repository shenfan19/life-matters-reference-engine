"""Pareto-front CSV writer shared by the CLI (sim_cli/output.py) and the GUI
backend (routes/optimizer.py:export_optimizer_csv).

Lives in sim_engine/src rather than sim_cli so the GUI backend never has to
import from sim_cli/ — the two ship as separate artifacts (CLI as a
