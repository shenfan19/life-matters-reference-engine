...
async def simulation_step(request: SimulationStepRequest):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    try:
        result = app_state.engine.batch_steps(
...
async def get_simulation_session(session_id: str):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    result = app_state.engine.get_session_info(session_id)
    if result['success']:
        return result
...
async def pause_simulation(request: SessionRequest):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    result = app_state.engine.pause_session(request.session_id)
    if result['success']:
        return result
...
async def resume_simulation(request: SessionRequest):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    result = app_state.engine.resume_session(request.session_id)
    if result['success']:
        return result
...
async def reset_simulation(request: SessionRequest):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    result = app_state.engine.reset_session(request.session_id)
    if result['success']:
        return result
...
async def export_simulation(request: SessionRequest):
    if app_state.engine is None:
        raise HTTPException(status_code=503, detail="Reference engine not initialized")
    result = app_state.engine.export_session_csv(request.session_id)
    if result['success']:
        return result
